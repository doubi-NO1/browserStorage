...
var supportWebsql = !!openDatabase,
  indexedDB = indexedDB || webkitIndexedDB || mozIndexedDB || OIndexedDB || msIndexedDB,
  ...
  return new Promise(function(resolve,reject){
    websql.transaction(function (t) {
      t.executeSql(sql, parm,
        function (t, results) {
          resolve(callback ? callback(results) : und);
        },
        function (t, error) {
          reject(error);
        });
  ...
  get: function (key) {
    return invoke(this.st, 'getItem', [this.name + '.' + key],
      function (value) {
        ...
        return val ? (val[1] ? val : val[0]) : und;
      });
  },
...
MemoryDriver.prototype={
  ...
  get: function (key) {
    return new Promise(function(resolve,reject){
      ...
      resolve(val ? val[1] === und ? val[0] : val : und);
    });
  },
